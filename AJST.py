...
class AJSTree:
    ...
    #   Compute the value of the node as the JS estimators of the targets
        ...
        self.tree[node_id]['value'] = self.compute_JS_estimators(y_m)

    def compute_JS_estimators(self, y_m):
        # Your original implementation
        y_true = np.array(y_m)
        # print(y_true)
        y_pred_sa = np.mean(y_true, axis=0)
    ...
    def _find_best_split(self, node_id):
        ...
        for feature_index in range(self.X.shape[1]):
            ...
            for split_value in unique_values:
                ...
                impurity = self._calculate_JS_impurity(left_samples, right_samples)
    ...
    def _calculate_JS_impurity(self, left_samples, right_samples):
        ...
        left_JS = self.compute_JS_estimators([self.y[i] for i in left_samples])
        right_JS = self.compute_JS_estimators([self.y[i] for i in right_samples])

        # Expand the JS estimators to match the shape of y_left and y_right for broadcasting
        left_JS_expanded = np.tile(left_JS, (len(left_samples), 1))
        right_JS_expanded = np.tile(right_JS, (len(right_samples), 1))
        ...
        SSE_left = np.sum((y_left - left_JS_expanded) ** 2)
        SSE_right = np.sum((y_right - right_JS_expanded) ** 2)
...
class AJSRF(BaseEstimator, RegressorMixin):
