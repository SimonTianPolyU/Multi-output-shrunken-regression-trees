...
class PSDTree(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        self._compute_sd_estimators(X, y)

    def _compute_sd_estimator(self, y_true, y_pred_sa):
        """
        ...
        adjustment_matrix = N_m * squared_diff_sum * np.identity(self.n_outputs_) + (self.n_outputs_ * trace_cov_matrix - sum_cov_matrix) * np.ones((self.n_outputs_, self.n_outputs_))/self.n_outputs_
    ...
    def _compute_sd_estimators(self, X, y):
        """
        ...
        for leaf in np.unique(leaf_indices):
            ...
            self.SD_estimators[leaf] = self._compute_sd_estimator(y_true_leaf, y_pred_sa_leaf)
    ...
    def predict(self, X):
        ...
        """
        leaf_indices = self.tree.apply(X)
        predictions = np.array([self.SD_estimators[leaf] for leaf in leaf_indices])
        return predictions
