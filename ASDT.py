...
class ASDTree:
    ...
    def compute_SD_estimators(self, y_m):
        ...
        # Calculate the SD estimator
        y_pred_SD = (1 / denominator) * adjustment_matrix.dot(y_pred_sa)

        return y_pred_SD  # Return the JS prediction
    ...
    def _find_best_split(self, node_id):
        ...
        for feature_index in range(self.X.shape[1]):
            ...
            for split_value in unique_values:
                ...
                impurity = self._calculate_SD_impurity(left_samples, right_samples)
    ...
    def _calculate_SD_impurity(self, left_samples, right_samples):
        ...
        left_SD = self.compute_SD_estimators([self.y[i] for i in left_samples])
        right_SD = self.compute_SD_estimators([self.y[i] for i in right_samples])

        # Expand the S estimators to match the shape of y_left and y_right for broadcasting
        left_SD_expanded = np.tile(left_SD, (len(left_samples), 1))
        right_SD_expanded = np.tile(right_SD, (len(right_samples), 1))
        ...
        SSE_left = np.sum((y_left - left_SD_expanded) ** 2)
        SSE_right = np.sum((y_right - right_SD_expanded) ** 2)
...
class ASDRF(BaseEstimator, RegressorMixin):
