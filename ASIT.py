...
class ASITree:
    ...
    #   Compute the value of the node as the JS estimators of the targets
        ...
        self.tree[node_id]['value'] = self.compute_SI_estimators(y_m)

    def compute_SI_estimators(self, y_m):
        ...
        adjustment_matrix = N_m * squared_diff_sum * np.identity(self.n_outputs_) + variance_sum * np.ones(
            (self.n_outputs_, self.n_outputs_)) * (self.n_outputs_ - 1) / self.n_outputs_

        # Calculate the MWS* estimator
        y_pred_SI = (1 / denominator) * adjustment_matrix.dot(y_pred_sa)

        return y_pred_SI  # Return the JS prediction
    ...
    def _find_best_split(self, node_id):
        ...
        for feature_index in range(self.X.shape[1]):
            ...
            for split_value in unique_values:
                ...
                impurity = self._calculate_SI_impurity(left_samples, right_samples)
    ...
    def _calculate_SI_impurity(self, left_samples, right_samples):
        # Extract the actual y values for left and right splits
        y_left = self.y[left_samples]
        y_right = self.y[right_samples]

        # Calculate the JS estimator predictions for left and right splits
        left_SI = self.compute_SI_estimators([self.y[i] for i in left_samples])
        right_SI = self.compute_SI_estimators([self.y[i] for i in right_samples])

        # Expand the S estimators to match the shape of y_left and y_right for broadcasting
        left_SI_expanded = np.tile(left_SI, (len(left_samples), 1))
        right_SI_expanded = np.tile(right_SI, (len(right_samples), 1))
        ...
        SSE_left = np.sum((y_left - left_SI_expanded) ** 2)
        SSE_right = np.sum((y_right - right_SI_expanded) ** 2)
...
class ASIRF(BaseEstimator, RegressorMixin):
